...
- **Local-first.** One relational SQLite database plus one blob-only media database. No server, sync, or required account. Capture, review, and lexical search work immediately offline; semantic search runs locally after a background model download. Optional R2 backup uses the network when enabled. Automatic update checks ask GitHub for the latest release shortly after launch and every six hours by default, and can be disabled in Settings without removing manual update checks.
