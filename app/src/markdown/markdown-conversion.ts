...
export function parseDaraMarkdown(
  ...
  const blocks = tree.children.flatMap((node) => {
    return node.type === 'definition'
      ? []
      : blockFromMarkdown(node, source, schema, definitions)
  })
  return schema.topNodeType.createAndFill(null, blocks.length ? blocks : null)!
}

export function serializeDaraMarkdown(document: ProseMirrorNode): string {
  const tree: Root = {
      ...
      return [
        type.create(
          ...
          node.children.map((item) =>
            listItemFromMarkdown(item, source, schema, definitions),
          ),
        ),
      ]
    }
...
function canonicalCodeLanguage(label: string | null | undefined): string | null {
  ...
  return sourceLabel
    ? (normalizeCodeLanguageLabel(sourceLabel) ?? sourceLabel)
    : null
}
...
function textFromMarkdownNode(node: Content): string {
  ...
  return 'children' in node
    ? (node.children as Content[]).map(textFromMarkdownNode).join('')
    : ''
}
      ...
      return [
        {
          ...
          start:
            node.type.name === 'ordered_list' ? (node.attrs.order ?? 1) : null,
          spread: false,
...
function tableToMarkdown(node: ProseMirrorNode): Table {
  return {
    ...
    align: node.firstChild
      ? Array.from({ length: node.firstChild.childCount }, () => null)
      : [],
