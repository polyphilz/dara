...
assert(
  arguments_.every((argument) => argument === ResumeArgument) &&
    arguments_.length <= 1,
...
const dmgPath = resolve(
  `src-tauri/target/release/bundle/dmg/Dara_${packageJson.version}_aarch64.dmg`,
)
const notarizationDirectory = resolve(
  'src-tauri/target/release/bundle/notarization',
)
const appArchivePath = join(notarizationDirectory, 'Dara.zip')
const dmgUploadPath = join(
  notarizationDirectory,
  `Dara_${packageJson.version}_aarch64.dmg`,
)
...
if (!resume || !existsSync(dmgSubmissionPath)) {
  ...
  submitAndRecord(
    DistributionArtifact.DiskImage,
    dmgUploadPath,
    dmgSubmissionPath,
    notarization,
  )
  ...
  assert(
    existsSync(dmgPath),
    `cannot resume because the signed disk image is missing: ${dmgPath}`,
  )
}
...
function buildSignedApplication(signingPolicy) {
  ...
  const temporaryDirectory = mkdtempSync(
    join(tmpdir(), 'dara-distribution-config-'),
  )
  const distributionConfigPath = join(
    temporaryDirectory,
    'tauri.distribution.conf.json',
  )
  writeFileSync(
...
function createSignedDiskImage(applicationPath, diskImagePath, signingPolicy) {
  const stagingDirectory = mkdtempSync(join(tmpdir(), 'dara-dmg-stage-'))
  mkdirSync(dirname(diskImagePath), { recursive: true })
  rmSync(diskImagePath, { force: true })
  try {
    run('ditto', [applicationPath, join(stagingDirectory, 'Dara.app')])
...
function retry(operation, label) {
  ...
  throw new Error(
    `${label} failed. Rerun pnpm release:resume:distribution.`,
    { cause: lastError },
  )
}
...
function runNotarytoolJson(arguments_, credentials) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `xcrun notarytool failed with exit code ${result.status ?? 'unknown'}`,
    )
  }
  try {
...
function preflight(notarization, signingPolicy) {
  ...
  assert(process.arch === 'arm64', 'distribution builds require arm64 macOS')
  assert(
  ...
  retry(
    () => runNotarytool(['history'], notarization),
    'validating Apple credentials',
  )
  console.info(
    `Apple credentials passed for ${basename(notarization.keyPath)}.`,
  )
}
