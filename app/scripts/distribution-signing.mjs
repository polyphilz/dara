...
export const distributionSigningPolicyPath = resolve(
  'src-tauri/distribution-signing.json',
)

export function readDistributionSigningPolicy() {
  const policy = JSON.parse(
    readFileSync(distributionSigningPolicyPath, 'utf8'),
  )
  ...
  assert(
    typeof policy.application?.signingIdentity === 'string' &&
      policy.application.signingIdentity.startsWith(
...
export function codeSigningRequirement(policy, identifier) {
  return [
    ...
    `certificate leaf[subject.OU] = ${quoteRequirementString(
      policy.application.teamIdentifier,
    )}`,
    `certificate leaf[subject.CN] = ${quoteRequirementString(
