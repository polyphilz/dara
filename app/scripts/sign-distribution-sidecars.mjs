...
for (const sidecar of Object.values(policy.sidecars)) {
  ...
  const manifest = JSON.parse(
    readFileSync(resolve(sidecar.pinManifestPath), 'utf8'),
  )
  const metadata = lstatSync(path)
  assert(metadata.isFile(), `${sidecar.component} is not a regular file`)
  assert(
    !metadata.isSymbolicLink(),
    `${sidecar.component} must not be a symbolic link`,
  )
  assert(
    (metadata.mode & 0o111) !== 0,
    `${sidecar.component} is not executable`,
  )
  assertEqual(
    metadata.size,
    manifest.binary.size,
    `${sidecar.component} unsigned byte length`,
  )
  assertEqual(
    sha256File(path),
    manifest.binary.sha256,
    `${sidecar.component} unsigned SHA-256`,
  )
